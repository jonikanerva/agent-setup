...
| `$VERIFY_CMD` | `pnpm verify` (type-check `tsc --noEmit` → lint → build → tests) |
...
_Starting points — adjust per the product._

- **API request p99:** 100 ms (per route, excluding upstream calls); **p50:** 30 ms.
- **Cold start (Node):** < 2 s.
- **Web initial bundle:** < 200 KB gzipped JS.
- **Time to Interactive (slow 4G):** < 3 s.
- Cache TTL behaviour is verified deterministically with `TestClock`, not wall-clock.
